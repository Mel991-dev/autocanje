  ...
  const cargarCarrito = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (response.data.items_problematicos && response.data.items_problematicos.length > 0) {
          const problemas = response.data.items_problematicos.map(item => item.nombre_producto).join(", ");
                      ...
                      <div key={item.id_carrito} className="producto-item">
                        <div className="producto-imagen">
                          {item.imagen_principal ? (
                            <img src={item.imagen_principal} alt={item.nombre_producto} />
                          ) : (
                        ...
                        <div className="producto-info">
                          <h3
                            ...
                            onClick={() => navigate(`/producto/${item.fk_producto}`)}
                          >
                            {item.nombre_producto}
                          </h3>
                          <p className="producto-meta">
                            Vendedor: {item.vendedor_nombre} {item.vendedor_apellido} • {item.categoria}
                          </p>
                          ...
                          <div className="producto-actions">
                            <div className="cantidad-controls">
                              <button
                                ...
                                onClick={() =>
                                  handleCantidadChange(item.id_carrito, item.cantidad, -1)
                                }
                              ...
                              <button
                                ...
                                disabled={item.cantidad >= item.stock || procesando}
                              >
                                <Plus size={16} />
                              </button>
                            ...
                            <div className="precio-info">
                              <div className="precio-total">{formatPrice(item.subtotal_item)}</div>
          ...
          <div className="carrito-resumen">
            <div className="card resumen-card">
              ...
              <div className="card-content">
                ...
                <div className="resumen-linea">
                  <span>Envío</span>
                  <span className={`valor ${totales.envio === 0 ? "gratis" : ""}`}>
                ...
                <div className="resumen-total">
                  <span>Total</span>
                  <span className="valor-total">{formatPrice(totales.total)}</span>
                </div>
                ...
                {totales.envio_gratis_en > 0 && (
                  <div className="envio-gratis-alert">
                    <AlertCircle size={16} />
                    <span>
                      Agrega {formatPrice(totales.envio_gratis_en)} más para envío gratis
                    </span>
                  </div>
                )}
              </div>
